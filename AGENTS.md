...
| `scripts/commands/config/env/remove.sh` | `cmd_config_env_remove` — removes an entire environment: `uninstall_remote` on all non-local servers, deletes ASCII art, removes `.servers[$e]` + `.env_colors[$e]` atomically, runs sync; git clone env protected (exit 1); self-removal: sync → config update → `uninstall_local`; `-e <env>` flag skips interactive menu; confirmation defaults N; sources `lib/uninstall.sh` |
| `scripts/lib/uninstall.sh` | `uninstall_remote <server>` (SSH heredoc, `SCRIPTS_DONE` sentinel, idempotent) + `uninstall_local` (local cleanup); sourced by `internal/uninstall.sh`, `commands/config/server/remove.sh`, and `commands/config/env/remove.sh`; idempotency guard `_FLEETMAN_UNINSTALL_LOADED` |
...
| `scripts/commands/config/server/add.sh` | `cmd_config_server_add` — adds one or more server FQDNs to `config.json .servers[$env]`; selects env once via colored menu; collects FQDNs via `read -r` loop (empty line to finish), validates each (FQDN regex + global duplicate check), writes each atomically; then calls `_deploy_key` per server (decrypts fleet password, runs `ssh-copy-id`, verifies key auth) — no sync; launches `fleetman sync` once after all keys deployed |
| `scripts/commands/config/server/remove.sh` | `cmd_config_server_remove` — removes a server: uninstalls fleetman via SSH (or locally for self-removal), updates `config.json`, deletes `$DATA_DIR/welcome_<short>.ascii`, runs `fleetman sync -q`; `select_menu_disabled` protects the git clone server (read from `$GIT_SERVER_FILE`); self-removal order: config → ASCII → sync → uninstall local (inverted to preserve sync access before fleetman is destroyed); `-e <env>` skips interactive env menu; confirmation defaults to N |
...
| `scripts/lib/ui.sh` | `prompt_response/select_menu/select_menu_disabled/build_server_labels/build_server_list_labels/prompt_sync_confirm/prompt_confirm` |
...
**`curl-pipe-bash` + `read`**: In the bootstrap block (`[[ ! -f "$0" ]]`), all interactive `read` calls must use `< /dev/tty` — without it, `read` consumes from stdin (the script pipe itself), reading a script line instead of user input and passing it directly to `git clone` as a URL.
...
- **`is_local_server <fqdn>`** — returns 0 if `<fqdn>` is the local server (exact FQDN match or short-name match against `$MASTER_HOST`); use instead of inline `[ "$server" = "$MASTER_HOST" ]` checks — short-name comparison handles cases where pods.json stores short names. Defined in `lib/auth.sh`. **In loops**: detect once before the loop (`_local_server="$_s"`) and compare `[[ "$_s" != "$_local_server" ]]` in the body — avoids a `hostname -f` fork per iteration.
...
- **`select_menu_disabled <labels_array> <disabled_indices_array>`** — arrow-key menu with greyed, non-selectable items; cursor automatically skips indices in the disabled array at startup and on navigation; sets global `SELECTED_IDX`; mock in unit tests as `select_menu_disabled() { SELECTED_IDX=0; }`. Defined in `lib/ui.sh`.
...
- **`prompt_confirm <question> [default]`** — prints `"  <question> [Y/n] "` (or `[y/N]` when `default=N`) to stdout, reads one line; returns 0 for yes, 1 for no; empty input uses the default (default `Y` when omitted — backward compatible); use `prompt_confirm "..." N` to default to no. Defined in `lib/ui.sh`.
...
- Commands with SSH/password flows require `prompt_confirm`, `check_sshpass`, `ask_password`, and `is_local_server` mocked in `setup()` — without them, tests reach interactive prompts and hang or fail non-deterministically in non-TTY bats.
...
- **Override `SCRIPTS_DIR` to isolate sync in unit tests**: Commands that call `bash "$SCRIPTS_DIR/bin/fleetman" sync` at runtime use the exported `SCRIPTS_DIR`. Set `SCRIPTS_DIR="$BATS_TEST_TMPDIR"` before `run` to redirect to a missing binary (hits the `warn "fleetman not found"` branch). To assert sync IS called once, create a mock: `printf '#!/bin/bash\necho "SYNC" >> "%s"\n' "$call_file" > "$BATS_TEST_TMPDIR/bin/fleetman"; chmod +x ...; SCRIPTS_DIR="$BATS_TEST_TMPDIR"` — then check `wc -l < "$call_file"`.
