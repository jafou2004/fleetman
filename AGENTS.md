...
| `scripts/commands/selfupdate.sh` | `cmd_selfupdate` — finds git clone server, git pull/checkout (tags/commits/branch/pin), runs migrations, triggers sync |
...
**curl-pipe-bash bootstrap** (`install.sh`): detect pipe execution with `[[ ! -f "$0" ]]` (`$0` is `bash` when piped from stdin, a real path otherwise). Clone repo, create symlink `~/scripts → ~/fleetman/scripts/`, then `exec bash "$_PROJECT_DIR/install.sh" "$@"` to re-exec from the real file — required so lib sourcing with `${BASH_SOURCE[0]}` works normally.
